  ...
  describe('GET /api/things', () => {
    ...
    context('Given there are things in the database', () => {
      ...
      it('responds with 200 and all of the things', () => {
        ...
        return supertest(app)
          .get('/api/things', function (res, body) {
            const serverData = res.body.map(data => {
              const newDate = new Date(data.date_created);
              data.date_created = new Date(newDate.setTime(newDate.getTime() - (5*60*60*1000)));
              return data;
            });
            expect(res.statusCode).to.equal(200);
            expect(serverData).to.deep.equal(expectedThings);
          });
      });
    });
  ...
  describe('GET /api/things/:thing_id', () => {
    ...
    context('Given there are things in the database', () => {
      ...
      it('responds with 200 and the specified thing', () => {
        ...
        return supertest(app)
          .get(`/api/things/${thingId}`, function (res, body) {
            const serverData = res.body.map(data => {
              const newDate = new Date(data.date_created);
              data.date_created = new Date(newDate.setTime(newDate.getTime() - (5*60*60*1000)));
              return data;
            ...
            expect(res.statusCode).to.equal(200);
            expect(serverData).to.deep.equal(expectedThing);
          })
          .set('Authorization', helpers.makeAuthHeader(testUsers[0]));
      });
    });
